...
class ShortenUrlUseCaseTest {
    ...
    @Test
    @DisplayName("Deve salvar, gerar shortCode e retornar nova URL quando a URL nao existir")
    void shouldCreateAndReturnNewUrlWhenNotExists() {
        String originalUrl = "https://github.com";
        ShortenUrlCommand command = new ShortenUrlCommand(originalUrl);

        Url initialSavedUrl = Url.builder()
                .id(62L)
                .originalUrl(originalUrl)
                .build();

        String expectedCode = Base62.encode(62L);
        Url finalSavedUrl = Url.builder()
                .id(62L)
                .originalUrl(originalUrl)
        ...
        when(urlRepository.save(any(Url.class)))
                .thenReturn(initialSavedUrl)  // primeiro save para gerar ID
                .thenReturn(finalSavedUrl);    // segundo save com shortCode

        ShortenUrlResponse response = shortenUrlUseCase.execute(command);

        assertThat(response).isNotNull();
        assertThat(response.getId()).isEqualTo(62L);
        assertThat(response.getOriginalUrl()).isEqualTo(originalUrl);
